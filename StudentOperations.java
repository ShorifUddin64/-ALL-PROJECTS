...
public class StudentOperations {
    ...
    public String searchByCourse(String courseName) {
        for (Student s : list) {
            // equalsIgnoreCase checks the name without worrying about Capital/Small letters
            if (s.getCourse().equalsIgnoreCase(courseName)) {
                return "Name: " + s.getName() + " | Section: " + s.getSection();
            }
        }
        return "No student found for this course.";
    }

    // Method to return all students as a single String
    public String displayAll() {
        if (list.isEmpty()) return "The list is currently empty.";
        
        String allData = "--- Registered Students ---\n";
        for (Student s : list) {
            allData += s.getName() + " - " + s.getCourse() + " (" + s.getSection() + ")\n";
        }
        return allData;
    }

    // Simplified update method (optional but good for your GUI buttons)
    public String update(String c, int cr, String s) {
        for (Student st : list) {
            if (st.getCourse().equalsIgnoreCase(c)) {
                // Assuming Student class has setters or you re-assign values
    ...
    // Simplified delete method (optional)
