...
public class Studentportal extends JFrame implements ActionListener {
    ...
    JLabel lblStatus1;
    JTextArea areaDisplay; // Replaced JLabel with JTextArea for easier English display
    ...
    public Studentportal() {
        ...
        // Input Fields
        add(new JLabel("Course Name:")).setBounds(30, 150, 100, 30);
        tfCourse = new JTextField(); tfCourse.setBounds(130, 150, 360, 30); add(tfCourse);

        add(new JLabel("Credit:")).setBounds(30, 200, 100, 30);
        tfCredit = new JTextField(); tfCredit.setBounds(130, 200, 70, 30); add(tfCredit);

        add(new JLabel("Section:")).setBounds(220, 200, 80, 30);
        ...
        // Main Display Area
        areaDisplay = new JTextArea("Welcome! Workspace Ready.");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            if (e.getSource() == btnAdd) {
                ...
                lblStatus1.setText(ops.add(name, course, cr, sec));
            } 
            else if (e.getSource() == btnSearch) {
                areaDisplay.setText("Searching for Course...\n" + ops.searchByCourse(tfSearch.getText()));
            } 
            else if (e.getSource() == btnUpdate) {
                int cr = Integer.parseInt(tfCredit.getText());
                lblStatus1.setText(ops.update(course, cr, sec));
            } 
            else if (e.getSource() == btnDelete) {
                lblStatus1.setText(ops.delete(course));
            } 
            else if (e.getSource() == btnDisplay) {
                areaDisplay.setText("Full Record for " + name + ":\n" + ops.displayAll());
            }
        } catch (Exception ex) {
            lblStatus1.setText("Error: Credit must be a valid number!");
        }
    }
